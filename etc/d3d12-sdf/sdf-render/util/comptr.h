...
namespace ant::sdf
{
	...
	class comptr 
	{
		public:
			...
			comptr(const comptr<CT>& other)
			{
				SetPointerAndAddRef(other.m_pointer);
			}
			comptr(comptr<CT>&& other) noexcept
			{
			...
			comptr<CT>& operator=(const comptr<CT>& other)
			{
				ClearPointer();
				SetPointerAndAddRef(other.m_pointer);
				return *this;
			}
			comptr<CT>& operator=(comptr<CT>&& other)
			{
				ClearPointer();
			...
			template<typename T>
			bool QueryInterface(comptr<T>& other, HRESULT* errorCode = nullptr)
			{
				if (m_pointer) 
				{
			...
			bool operator==(const comptr<CT>& other)
			{
